...
# 시총 Top 10 기업 (2024년 기준, 2010년 이후 진입 기업 구분)
ALL_COMPANIES = {
    "📌 [기존 강자] Apple": "AAPL",
    "📌 [기존 강자] Microsoft": "MSFT",
    "📌 [기존 강자] Alphabet": "GOOGL",
    "📌 [기존 강자] Amazon": "AMZN",
    "🚀 [New Entry] NVIDIA": "NVDA",
    "🚀 [New Entry] Tesla": "TSLA",
    "🚀 [New Entry] Meta": "META",
    "📌 [기존 강자] Berkshire Hathaway": "BRK-B",
    "🚀 [New Entry] Taiwan Semiconductor": "TSM",
    "📌 [기존 강자] Visa": "V"
}
...
def main():
    ...
    selected_companies = st.sidebar.multiselect(
        ...
        default=list(ALL_COMPANIES.keys())[:3]
    )
    ...
    with st.spinner("데이터를 불러오는 중..."):
        ...
        for label in selected_companies:
            ticker = ALL_COMPANIES[label]
            data = get_stock_data(ticker, period_options[selected_period])
            info = get_company_info(ticker)
            
            if data is not None and not data.empty:
                stock_data[label] = data
                company_info[label] = info
    ...
    if chart_type == "라인 차트":
        ...
        for i, (label, data) in enumerate(stock_data.items()):
        ...
        # 캔들스틱 차트는 첫 번째 기업만 표시
        if len(selected_companies) > 1:
            st.info("캔들스틱 차트는 한 번에 하나의 기업만 표시됩니다. 첫 번째 선택된 기업을 표시합니다.")
        
        label = selected_companies[0]
        data = stock_data[label]
        ...
        fig.update_layout(
            title=f"{label} 캔들스틱 차트 - {selected_period}",
            xaxis_title="날짜",
            yaxis_title="주가 (USD)",
            height=600,
            template='plotly_white'
